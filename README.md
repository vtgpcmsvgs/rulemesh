...
- Outlook 邮件、精确共享认证与资源直连，不放宽 Microsoft 根域。WPS 统一直连、Microsoft Store 固定美国，Notion 复用香港自动选择，香港证券与用户指定 GoDaddy、尊嘉证券、supado.com 保留香港。详见 [Notion 网页优化](docs/notion-network-optimization.md)。
...
机场策略与配置精简：三份私人 Surge 的八个机场手动组分别保留并接回选择入口，不按规则引用数量删除。Personal 爱思入口集中为 `direct/aisi_direct`；Apple 更新和 Google Play 复用既有规则集；Surge AI DNS 的美国出口集中为 `region/us/ai_dns_us`。Mihomo 保持已有组与 DNS 代理参数，不扩大业务范围。设备地址及订阅端点继续只在私人仓库维护，详见 [性能基线](docs/performance-baseline.md)。
