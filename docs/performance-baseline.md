...
- Surge 只清理随 AWS/链式功能停用的设备专用组。机场手动组是独立选择功能，即使没有规则引用也必须保留；三份私人 Surge 各保留八组，保持原订阅与过滤器，设为可见并接入手动选择入口。两份 Mihomo 原有九组未删除，本轮保留。
...
- 上轮把没有规则引用的机场组误判为无用设备组；现已分开维护，检查器要求三份私人 Surge 的机场保护块各有八个有效且可见的独立 select 组，并验证手动入口引用。机场增减时同步调整该基线及检查，不能静默删组。
