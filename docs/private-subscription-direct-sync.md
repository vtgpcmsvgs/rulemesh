...
- 同时检查源文件与五份目标中是否残留已废弃端点、重复 provider、旧 provider 缓存或孤立代理组引用
...
- 三份私人 Surge 当前保留七个机场手动组；新增组须沿用 `✈️ 名称` 格式、可见并接入选择入口，重命名同步全部 include-other-group 引用。若新增订阅尚未包含在旧聚合中，可通过 `include-other-group` 同时接入聚合手动节点列表及各自动组，沿用各组的地区过滤器。Surge 会按节点名去重，不重复添加独立直连订阅来源。
...
- 两份 Mihomo 新增同名 provider，保持后台 DIRECT、已验证的 User-Agent、独立缓存路径及机场前缀，补齐所有既有 use 列表。桌面健康检查 300 秒、安卓 600 秒；新增机场不改变地区例外、Google 下载保护或业务放行范围。
