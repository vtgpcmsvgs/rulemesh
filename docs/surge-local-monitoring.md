...
- Google 自动入口、ChatGPT 美国入口是否因为当前策略或节点短时不可用而出现几十秒恢复窗口
...
2026-09-09 性能基线允许普通业务使用国内 DNS；AI 保留美国专用解析。调查以实际 DNS 出口与耗时为准，不能依据旧版海外 DNS 假设提出回滚。
...
监控建议文案已对齐 2026-09-09 基线，采集方式、隐私、只读权限和两阶段审批保持原样。源程序变更后必须在实际 Mac 上重新运行安装器同步运行副本；Windows 仓库更新不会自动更新 Mac 的已安装服务。
